d3.csv("vuldb.csv").then(function (data) {
  ...
  function runEnter() {
    ...
    for (var i = 0; i < filteredData.length; i++) {
      ...
      d3.select("tbody").insert("tr").html("<td>"+"<div id="accordion" class="collapse">" +(output[i]['description'])+"<br>" +(output[i]['impact'])+"<br>" +(output[i]['likelihood'])+"<br>" +(output[i]['remediation'])+"<br>" +(output[i]['reference'])+"<br>" +(output[i]['severity'])+"<br>" +(output[i]['rating'])+"<br>" +(output[i]['cvss'])+"<br>" +(output[i]['database'])+"<br>" +(output[i]['source'])+"</div>"+"</td>")
    }
  };
